...
임시로 **160W까지 낮췄더니 원인 모를 다운은 멈췄는데, 이번엔 작업 속도가 눈에 띄게 느려졌습니다.** 안정성과 성능을 맞바꾼 셈인데, 어느 지점이 적정선인지는 **아직 확정하지 못했어요.** 지금은 이 명령으로 실제 병목이 전력 제한인지를 확인하고 있습니다.
...
![160W로 전력 제한을 건 상태의 nvidia-smi 출력. 온도 75도, 사용률 79%에서 안정적으로 도는 모습](images/nvidia-smi-160w.png)
...
중고 V100은 가격만 보면 매력적이지만, 실제로는 **블로워 팬이 달린 SXM2 변환보드를 골라 냉각을 해결하고, 전력 스파이크가 서버 다운으로 이어지는지까지 확인하고, 재부팅마다 초기화되는 설정을 자동화하는** 작업이 뒤따릅니다. 그 수고를 감수할 만한 이유는 HBM2 대역폭이 주는 체감 성능이고, 감수해야 할 한계는 언젠가 끊길 드라이버 지원과 최신 정밀도 포맷 미지원입니다. 저도 지금은 안정성을 우선해 160W에 머물러 있고, 원인을 더 확인한 뒤에 180W로 올릴지 결정할 계획이에요 — 아직 다 풀지 못한 문제도 있다는 걸 솔직히 남겨둡니다. 사기 전에 이 편차를 알고 있으면, 조립하고 나서 당황할 일은 줄어듭니다.
